...
teste('calcularVencimentoVenda: à vista não tem prazo nem vencimento', () => { const v = sandbox.calcularVencimentoVenda({ forma_pagamento:'avista' }); iguais(v.formaPagamento, 'avista'); verdadeiro(v.vencimento === null); });
teste('calcularVencimentoVenda: a prazo calcula o vencimento a partir da Data NF Saída + dias', () => { const v = sandbox.calcularVencimentoVenda({ forma_pagamento:'aprazo', prazo_dias:'30', nf_saida_data:'2026-08-01' }); iguais(v.prazoDias, 30); iguais(v.vencimento.toISOString().split('T')[0], '2026-08-31'); });
teste('calcularVencimentoVenda: parcelado calcula vencimento de cada parcela a partir da Data NF Saída', () => { const v = sandbox.calcularVencimentoVenda({ forma_pagamento:'parcelado', nf_saida_data:'2026-08-01', parcelas:[{dias:'30',percentual:'30'},{dias:'60',percentual:'30'},{dias:'90',percentual:'40'}] }); iguais(v.formaPagamento, 'parcelado'); iguais(v.parcelas.length, 3); iguais(v.parcelas[0].vencimento.toISOString().split('T')[0], '2026-08-31'); iguais(v.parcelas[1].vencimento.toISOString().split('T')[0], '2026-09-30'); iguais(v.parcelas[2].vencimento.toISOString().split('T')[0], '2026-10-30'); iguais(v.parcelas[2].percentual, 40); });
teste('renderFechamentoInfo: mostra o prazo negociado (À Vista ou A Prazo + vencimento) na linha de cada cliente', () => { const p = { nf_saida_valor:100000, produtos_json: JSON.stringify([{descricao:'X',quantidade:10}]), real_json:{fob:5000}, real_cambio:5, vendas_json: JSON.stringify([{ cliente:'CLIENTE A', itens:[{descricao:'X',quantidade:10}], nf_saida_valor:100000, nf_saida_data:'2026-08-01', forma_pagamento:'aprazo', prazo_dias:'30' }]) }; const html = sandbox.renderFechamentoInfo(p); verdadeiro(html.includes('A Prazo (30d)'), 'deveria mostrar a forma de pagamento A Prazo com os dias'); verdadeiro(html.includes('31/08/2026'), 'deveria mostrar a data de vencimento calculada'); });
teste('renderFechamentoInfo: mostra cada parcela (dias, %, vencimento) quando a venda é parcelada', () => { const p = { nf_saida_valor:100000, produtos_json: JSON.stringify([{descricao:'X',quantidade:10}]), real_json:{fob:5000}, real_cambio:5, vendas_json: JSON.stringify([{ cliente:'CLIENTE B', itens:[{descricao:'X',quantidade:10}], nf_saida_valor:100000, nf_saida_data:'2026-08-01', forma_pagamento:'parcelado', parcelas:[{dias:'30',percentual:'50'},{dias:'60',percentual:'50'}] }]) }; const html = sandbox.renderFechamentoInfo(p); verdadeiro(html.includes('Parcelado:'), 'deveria indicar forma de pagamento parcelada'); verdadeiro(html.includes('30d (50%)'), 'deveria mostrar a primeira parcela'); verdadeiro(html.includes('31/08/2026'), 'deveria mostrar o vencimento da primeira parcela'); verdadeiro(html.includes('60d (50%)'), 'deveria mostrar a segunda parcela'); verdadeiro(html.includes('30/09/2026'), 'deveria mostrar o vencimento da segunda parcela'); });

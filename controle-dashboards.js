...
function renderDashExecutivo(){
  ...
  const nfSaidaPeriodo = ativos.filter(p=>p.nf_saida_data&&!ehCfopRemessaEstoque(p.nf_saida_cfop)&&parseDataLocal(p.nf_saida_data)>=mesIni&&parseDataLocal(p.nf_saida_data)<=mesFim);

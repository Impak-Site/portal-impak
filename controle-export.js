...
async function exportarRelatorioNarcelioExcel(rel){
  ...
  try{
    ...
    titulo.fill = { type:'pattern', pattern:'solid', fgColor:{argb:CORES.AZUL} };
    ...
    linhas.forEach(([label, valor]) => {
      ...
      row.getCell(1).fill = { type:'pattern', pattern:'solid', fgColor:{argb:CORES.AZUL} };
      row.getCell(1).alignment = { vertical:'middle', horizontal:'left' };
      row.getCell(2).font = { name:'Calibri', bold:true, size:11, color:{argb:CORES.BRANCO} };
      row.getCell(2).fill = { type:'pattern', pattern:'solid', fgColor:{argb:CORES.AZUL} };

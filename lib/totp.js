...
function verify({ secret, token, digits = 6, period = 30, algorithm = 'sha1', epoch = Date.now(), window = 1 } = {}) {
  ...
  // Exige exatamente `digits` dígitos. Além de rejeitar entrada obviamente
  // inválida (letras, vazio, etc.), isso também protege o timingSafeEqual
  // logo abaixo: ele lança RangeError se os dois buffers não tiverem o
  // mesmo tamanho, então um token mais longo/curto que `digits` precisa
  // ser descartado ANTES de chegar lá -- não só confiar que quem chama
  // esta função já validou isso (defesa em profundidade).
  if (!/^\d+$/.test(t) || t.length !== digits) return { valid: false };
  const counter = currentCounter(period, epoch);
  for (let errorWindow = -window; errorWindow <= window; errorWindow++) {
    const candidate = hotp(secret, counter + errorWindow, digits, algorithm);
    if (crypto.timingSafeEqual(Buffer.from(candidate), Buffer.from(t))) {
